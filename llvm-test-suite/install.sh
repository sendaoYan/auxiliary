...
{
    cd ${BM_RUN_DIR}/
    rm -rf ${BM_RUN_DIR}/.shrc
    echo "export USER=$USER" >> ${BM_RUN_DIR}/.shrc
    echo "export PASS=$PASS" >> ${BM_RUN_DIR}/.shrc
    sed -i "s|(|\\\\(|g" ${BM_RUN_DIR}/.shrc
    sed -i "s|)|\\\\)|g" ${BM_RUN_DIR}/.shrc
}
...
installCompiler()
{
    ...
    export arch=`arch`
    echo "export arch=${arch}" >> ${BM_RUN_DIR}/.shrc
    ...
    prepend PATH ${ROOT}/compiler/${testsuite}/tools/${arch}

    #get gcc-ref binary
    cd ${BM_RUN_DIR}
    GCC_REF_VERSION=12.2.0
    wget_wrapper "https://.../llvm/binary/gcc-${GCC_REF_VERSION}-binary-${arch}.tar.xz" gcc-${GCC_REF_VERSION}-binary-${arch}.tar.xz
    ...
    if cat /etc/os-release | grep PRETTY_NAME | grep "CentOS Linux 7" ; then
        ...
        echo "source /opt/rh/devtoolset-10/enable" >> ${BM_RUN_DIR}/.shrc
    fi
}
